...
### DESCRIPTION ################################################################
#  Specific script only runs on LUAD data as stratifies data by smoking status;
#  downloads TCGAbiolinks data for LUAD tumor type and produce an oncoprint for 
#  the top most mutated genes for each LUAD smoking status subtype.
...
never.smokers <- smoke.complete.cases[which(smoke.complete.cases$subtype_Smoking.Status == "NeverSmoked"),]
smokers <- smoke.complete.cases[which(smoke.complete.cases$subtype_Smoking.Status == "Smokes"),]
